 ...
 * ✅ NO rate limiting (cron-controlled)
 * 
 * ALLOWED TRIGGERS:
 * ✅ AWS EventBridge cron (every 5 minutes)
 * ✅ Manual curl with secret key (emergency only)
 * ❌ NEVER from UI buttons
 * ❌ NEVER from public endpoints
 * 
 * SAFETY RULES:
...
/**
 * POST /api/internal/campaigns/execute-batch
 * 
 ...
 * 
 * SECURITY:
 * Requires x-internal-key header with INTERNAL_OPERATIONS_KEY
 * 
 * USAGE:
...
export async function POST(req: NextRequest) {
  console.log('🚀 [Execute Batch API] Internal request received');
  
  try {
    // 🔒 SECURITY: Verify internal authentication
    verifyInternalRequest(req);
    console.log('✅ [Execute Batch API] Internal auth verified');
